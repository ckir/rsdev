...
pub async fn run(config: Config, app_state: AppState, mut shutdown: broadcast::Receiver<()>) {
    let dataflow_check_interval_seconds = config.dataflow_check_interval_seconds.unwrap_or(10);
    let dataflow_inactivity_threshold_seconds = config.dataflow_inactivity_threshold_seconds.unwrap_or(60);

    let mut check_interval = interval(Duration::from_secs(dataflow_check_interval_seconds));
        ...
        tokio::select! {
            ...
            _ = check_interval.tick() => {
                ...
                if app_state.has_active_subscriptions().await {
                    if (current_time - last_data_time) > Duration::from_secs(dataflow_inactivity_threshold_seconds) {
                        log::warn!(
                        ...
                        // when no data is received. We need to notify clients.
